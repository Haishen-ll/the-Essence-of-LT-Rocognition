...
parser.add_argument('--log_all_2',type=str, default='./log_all')
...
def main():
    ...
    for i in range(50):
        print("Iter: {} for training".format(i))
        num_list = []
        num_list.append(5000)
        num_list = num_list*9
        num_list.append(5000-100 * i)
        # print(num_list)
        args.store_name = '_'.join([args.dataset, args.arch, args.loss_type, args.train_rule, args.imb_type, str(args.imb_factor), args.exp_str, "iter" + str(i)])
        prepare_folders(args)
        main_worker(args.gpu, ngpus_per_node, num_list, args)


def main_worker(gpu, ngpus_per_node, img_num_list, args):
    global best_acc1
    args.gpu = gpu
    ...
    if args.dataset == 'cifar10':
        train_dataset = IMBALANCECIFAR10(root='/data/liulei/cifar10/', imb_type=args.imb_type, imb_factor=args.imb_factor, rand_number=args.rand_number, train=True, download=True, transform=transform_train, img_num_list=img_num_list)
        test_dataset = datasets.CIFAR10(root='/data/liulei/cifar10/', train=False, download=True, transform=transform_test)
        # test_dataset = IMBALANCECIFAR10(root='D:/dataset/CIFAR10', imb_type=args.imb_type, imb_factor=args.imb_factor, rand_number=args.rand_number, train=True, download=True,
    ...
    # elif args.dataset == 'tiny':
    #     train_dataset = LT_TINYIMAGENET(root='/home/public_data/liulei/tiny_imagenet/',  imb_type=args.imb_type, imb_factor=args.imb_factor, rand_number=args.rand_number, train=True, transform=transform_train)
    #     test_dataset = LT_TINYIMAGENET(root='/home/public_data/liulei/tiny_imagenet/',  imb_type=args.imb_type, imb_factor=args.imb_factor, rand_number=args.rand_number, train=False, transform=transform_test)
    else:
    ...
    log_all = open(os.path.join(args.log_all_2, 'log_all.csv'), 'w')
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        acc1, output, out_cls_acc = validate(test_loader, model, criterion, epoch, args, log_testing, tf_writer)
    ...
    if log_all is not None:
        log_all.write(output + '\n')
        log_all.write(out_cls_acc + '\n')
        log_all.flush()
...
def train(num_classes, train_dataset, train_loader, model, criterion, model_opt, epoch, args, log, tf_writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # new_target = torch.Tensor().cuda(args.gpu)
        # for tar_item in target_list:
        #     new_target = torch.cat((new_target, torch.Tensor(args.label_model[tar_item]).cuda(args.gpu)),0)
        # new_target = new_target.view(target.shape[0], -1)
...
def validate(val_loader, model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    return top1.avg, output, out_cls_acc
