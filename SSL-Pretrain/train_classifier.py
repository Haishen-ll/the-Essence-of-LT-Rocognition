...
parser.add_argument('--train_rule', default='Reweight', type=str, choices=['None', 'Resample', 'Reweight', 'DRW'])
parser.add_argument('--rand_number', default=0, type=int, help='fix random number for data sampling')
parser.add_argument('--exp_str', default='Simsiam_cls', type=str, help='(additional) name to indicate experiment')
...
def main_worker(gpu, args):
    ...
    #     filename = 'checkpoint/cifar10_resnet32_CE_None_exp_0.01_backbone/ckpt.pth.tar'
    ...
    #     model.load_state_dict(model_dict)
        if args.pretrained:
            ...
            for k, v in pretrained_dict.items():
                if "backbone" in k:
                    new_dict.update({k.replace("backbone.", ""): v})
    ...
    optimizer = torch.optim.SGD(params_list, args.lr,
